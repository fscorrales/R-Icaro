...
bsModalRegistroServer <- function(id, toggleOpen, DatosOriginales) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    shinyBS::addPopover(session, ns("iconCUIT"), "Opciones Listado CUIT:", 
                        ...
                        content = paste0(
                          "<ul>",
                          "<li><strong>Al seleccionar un CUIT</strong> del listado, ",
                          "<strong>por número de CUIT o Razón Social</strong>, automáticamente ",
                          "se completará el listado con las obras que haya ejecutado el mismo</li>",
                          "<li>Si el <strong>CUIT queda en blanco</strong> se blanqueará el campo Obra ",
    ...
    CamposEstructura <- c("FechaRegistro", "NroComprobanteRegistro", "ComprobanteRegistro", 
                          "TipoRegistro", "DescObraRegistro", "CUITRegistro", 
    ...
    DatosFormulario <- reactive({
      ...
      data["CUITRegistro"] <- str_split_fixed(data["CUITRegistro"], " - ", 2)[1]
      data
    ...
    observeEvent(toggleOpen(), {
      ...
      if (!is.null(NroCUIT)) {
        NroCUITyDesc <- FiltrarBD(
          "SELECT DISTINCT CUIT, Descripcion FROM PROVEEDORES WHERE CUIT = ?",
          params = NroCUIT
        )
        NroCUITyDesc <- str_c(NroCUITyDesc[["CUIT"]], NroCUITyDesc[["Descripcion"]], sep = " - ") 
      }
      ...
      CUITRegistro <- dbGetQuery(
        con, "SELECT CUIT, Descripcion FROM PROVEEDORES")
      CUITRegistro <- str_c(CUITRegistro[["CUIT"]], CUITRegistro[["Descripcion"]], sep = " - ")
      ...
      if (IDModificacion() == 1) {
        ...
        if (Origen == "") {
          ...
          if (Origen == "Obras") {
            ...
            DescObraRegistro <- FiltrarBD(
              paste0(
              "SELECT DISTINCT Obra as Descripcion, Imputacion, Partida ",
              "FROM CARGA WHERE CUIT = ? UNION ",
              "SELECT DISTINCT Descripcion, Imputacion, Partida ",
              "FROM OBRAS WHERE CUIT = ?"
              ),
              params = list(NroCUIT, NroCUIT)
            ) %>% unique()
          ...
          updateSelectizeInput(session, inputId = ('CUITRegistro'),
                               ...
                               selected = NroCUITyDesc, server = TRUE)
    ...
    observeEvent(input$CUITRegistro, {
      ...
      CUITSinDescripcion <- str_split_fixed(input$CUITRegistro, " - ", 2)[1]
