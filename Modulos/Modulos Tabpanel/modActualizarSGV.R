...
ActualizarTesoreriaSIIFUI <- function(id) {
  ...
  tabPanel("Sistema Gestión Viviendas (SGV)",
           useShinyjs(),
           sidebarLayout(
                          ...
                          tabsetPanel(id = ns("switcher"), type = "hidden",
                            tabPanel("1",
                                     ...
                                     p(style="text-align: center;",
                                       strong("Facturado Recuperos")),
                                     p(style="text-align: center;",
                                       strong("archivo exportable Recuperos GV")),
                       ...
                       fluidRow(
                         ...
                         column(11, 
                                tabsetPanel(id = ns("controller"),
                                            tabPanel("Facturado Recuperos", value = "1", 
                                                     DataTableUI(ns("DTActualizaFacturadoRecuperosSGV"))),
                                            tabPanel("Recaudado Recuperos", value = "2", 
                                                     DataTableUI(ns("DTActualizaRecaudadoRecuperosSGV")))
...
ActualizarTesoreriaSIIFServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    DataTableServer("DTActualizaFacturadoRecuperosSGV", FacturadoRecuperosSGV.df)

    DataTableServer("DTActualizaRecaudadoRecuperosSGV", RecaudadoRecuperosSGV.df)
